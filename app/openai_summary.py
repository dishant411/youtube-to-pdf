...
def translate_summary_to_english(
    title: str,
    canonical_url: str,
    summary_text: str,
...
def summarize_transcript(
    ...
    if not _is_english_language(language):
        print(
            "Summarizing {language} transcript before English translation...".format(
                language=language or "non-English"
            ),
            file=sys.stderr,
        )
    ...
    summary_text, resolved_model = request_summary_text(
        prompt,
        model=resolved_model,
        max_output_tokens=final_summary_max_output_tokens,
    )
    if _is_english_language(language):
        return summary_text, resolved_model
    return translate_summary_to_english(
        title=title,
